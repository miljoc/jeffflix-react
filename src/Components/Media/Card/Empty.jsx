...
const EmptyArticle = styled.div`
  ...
  ${media.tablet`
    ...
    max-width: ${(props) =>
        props.wide ? 'calc(50% - 3rem)' : 'calc(25% - 3rem)'};
  `}

  ${media.desktop`
    width: ${(props) =>
        props.wide ? 'calc(33.3333% - 3rem)' : 'calc(25% - 3rem)'};
    max-width: ${(props) =>
        props.wide ? 'calc(33.3333% - 3rem)' : 'calc(25% - 3rem)'};
  `}
  ...
  ${media.large`
    width: ${(props) =>
        props.wide
            ? props.theme.wideCard.width
            : (props) => props.theme.card.width};
    max-width: ${(props) =>
        props.wide
            ? props.theme.wideCard.maxWidth
            : (props) => props.theme.card.maxWidth};
    margin:0 1.5rem;
  `}
`;

const EmptyArticles = ({ length, wide }) => {
    const EmptyList = [...Array(length)].map(() => (
        <EmptyArticle wide={wide} key={Math.random()} />
    ));

    return <Fragment>{EmptyList}</Fragment>;
};
